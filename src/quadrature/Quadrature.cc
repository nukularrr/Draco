 ...
 *          in the ordinate set for each level? This argument is ignored for 
 *          Cartesian geometries.
 * \param include_extra_directions Should extra starting directions
 *          be included in the ordinate set for each level?
 */
vector<Ordinate> Quadrature::create_ordinates(unsigned const dimension, Geometry const geometry,
...
vector<Ordinate> Quadrature::create_ordinates(unsigned const dimension, Geometry const geometry,
  ...
  vector<Ordinate> Result = create_ordinates_(
...
Quadrature::create_ordinate_set(unsigned const dimension, Geometry const geometry,
  ...
  vector<Ordinate> ordinates = create_ordinates_(
      dimension, geometry, norm, include_starting_directions, include_extra_directions);
...
void Quadrature::add_2D_starting_directions_(Geometry const geometry,
  ...
  if (add_starting_directions) {
    if (geometry == rtt_mesh_element::AXISYMMETRIC) {
      ...
      for (vector<Ordinate>::iterator a = ordinates.begin(); a != ordinates.end(); ++a) {
...
//------------------------------------------------------------------------------------------------//
void Quadrature::map_axes_(unsigned const mu_axis, unsigned const eta_axis, vector<double> &mu,
...
Quadrature::create_ordinate_space(unsigned const dimension, Geometry const geometry,
  ...
  vector<Ordinate> ordinates = create_ordinates(dimension, geometry,
                                                1.0, // hardwired norm
                                                mu_axis, eta_axis,
  ...
  switch (qim) {
  case SN:
    Result.reset(new Sn_Ordinate_Space(dimension, geometry, ordinates, moment_expansion_order,
                                       include_extra_directions, ordering));
    break;

  case GQ1:
    Result.reset(new Galerkin_Ordinate_Space(dimension, geometry, ordinates, quadrature_class(),
                                             number_of_levels(), moment_expansion_order, GQ1,
                                             include_extra_directions, ordering));
    break;

  case GQ2:
    Result.reset(new Galerkin_Ordinate_Space(dimension, geometry, ordinates, quadrature_class(),
                                             number_of_levels(), moment_expansion_order, GQ2,
                                             include_extra_directions, ordering));
    break;
