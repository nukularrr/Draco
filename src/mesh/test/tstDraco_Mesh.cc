...
void spherical_mesh_1d(rtt_c4::ParallelUnitTest &ut) {
  ...
  std::shared_ptr<Draco_Mesh> mesh_nobdy(new Draco_Mesh(
      dimension, geometry, cell_type, cell_to_node_linkage, {}, {}, {},
...
void cartesian_mesh_2d(rtt_c4::ParallelUnitTest &ut) {
  ...
  // instantiate a mesh with face type input
  std::shared_ptr<Draco_Mesh> mesh_alt(new Draco_Mesh(
      mesh_iface_alt.dim, geometry, mesh_iface_alt.cell_type,
      mesh_iface_alt.cell_to_node_linkage, mesh_iface_alt.side_set_flag,
      mesh_iface_alt.side_node_count, mesh_iface_alt.side_to_node_linkage,
      mesh_iface_alt.coordinates, mesh_iface_alt.global_node_number, {}, {}, {},
      {}, mesh_iface_alt.face_type));
  ...
  // get the alternate layout
  const Draco_Mesh::Layout &layout_alt = mesh_alt->get_cc_linkage();

  // check that the layout has been generated
  FAIL_IF_NOT(layout.size() == mesh_iface.num_cells);
  FAIL_IF_NOT(layout_alt == layout);
  ...
  // check that each cell has the correct neighbors
  {
    ...
    for (unsigned cell = 0; cell < mesh_iface.num_cells; ++cell) {
      ...
      // check alternate mesh
      const size_t num_faces_alt = layout_alt.at(cell).size();
      FAIL_IF_NOT(num_faces_alt == num_faces);

      // check that cell-cell layout of alternate mesh is correct
      for (unsigned face = 0; face < num_faces; ++face) {
        FAIL_IF_NOT(layout_alt.at(cell)[face].first ==
                    test_cell_map[cell][face]);
        FAIL_IF_NOT(layout_alt.at(cell)[face].second ==
                    layout.at(cell)[face].second);
      }
    }
  }
