...
void toss_cookies(std::string const &cond, std::string const &file,
                  int const line, bool const print_stack) {
  throw assertion(cond, file, line, print_stack);
}
...
void check_cookies(bool const cond, char const *const cond_text,
  ...
  if (!cond)
    throw assertion(cond_text, file, line, false);
}
...
void show_cookies(std::string const &cond, std::string const &file,
                  int const line) {
  std::cerr << assertion::build_message(cond, file, line, false) << std::endl;
}
 ...
 * \param msg Additional message explaining the failed condition.
 * \param file File where the condition is checked.
 * \param line Line where the condition is checked.
 *
 * \return Cannot return
 *
 * \throw std::ios::failure if I/O fails
 * \throw std::bad_alloc if storage is not available to construct the message
 * \throw asssertion is the expected effect of this function
 *
 * \deprecated Supports the deprecated Insist macro.
 */
