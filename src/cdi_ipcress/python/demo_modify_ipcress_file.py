...
# brief  A script that uses the ipcress_reader.py functions to write new opcaity values to an
#        IPCRESS file data for a range of temperatures.
...
ipcress_data, materials = ip_reader.get_property_map_from_ipcress_file(ipcress_file)
...
# get the number of grid points in the opacity data (your new data need to be the same size)
...
ip_reader.write_information_to_file(new_ipcress_file, material_ID, scat_mat_property, new_rsmg)

# write the limited absorption data to the file
ip_reader.write_information_to_file(new_ipcress_file, material_ID, abs_mat_property, new_mgar_grid)

# get data dictionary and material IDs from the new file
new_ipcress_data, new_materials = ip_reader.get_property_map_from_ipcress_file(new_ipcress_file)
...
# ------------------------------------------------------------------------------------------------ #
# End demo_modify_ipcress_file.py
# ------------------------------------------------------------------------------------------------ #
