 ...
 * object), a material identifier, an energy model (already selected since this
 * is a Gray Opacity class), a physics model and a reaction type.
 *
 * This is a concrete class derived from cdi/GrayOpacity.  This class allows to
 ...
 * provide a mechanism for accessing data in IPCRESS files and works by calling
 * the Ipcress library provided by X-5.  The IpcressGrayOpacity constructor
 * expects four arguments: a hook to IPCRESS data file (spIpcressFile), a
 * material identifier, an opacity model (Rosseland or Plank) and an opacity
 * reaction specifier (total, scattering or absorption).  Once constructed, this
 * object allows the client to access any data found in the IPCRESS file for
 * that one material.  The client code will need to create a separate
 * IpcressGrayOpacity object for each material that it needs information
 * about. Multiple opacity objects can exist per IPCRESS file.
 *
 ...
 * IpcressGrayOpcity object calls the appropriate IPCRESS library routine, which
 * in turn, interpolates on the data cached in the IpcressDataTable object.
 *
 * \example cdi_ipcress/test/tIpcressOpacity.cc
 *
 * Example of IpcressGrayOpacity usage independent of CDI.  In this example we
 * construct a IpcressGrayOpacity object for the material Aluminum (matID=10001
 * in our example IPCRESS file).  We then use the IpcressGrayOpacity object to
 * compute a Rosseland Gray opacity value for a specified material, temperature
...
public:
   ...
   *
   * This constructor unpacks a IpcressGrayOpacity object from a state attained
   * through the pack function.
   *
   * \param packed vector<char> of packed IpcressGrayOpacity state; the packed
   ...
   *
   * This is required to correctly release memory when a IpcressGrayOpacity is
   ...
   *        iterators are required to be the same length.  The opacity iterator
   *        should also have this same length.
   *
   ...
   *     list of densities.
   * \param densityLast container that holds a list of temperatures.  \param
   *     opacityFirst The beginning position of a STL container into which
   ...
   *     opacity values (corresponding to the provided temperature and density
  ...
  /*!
   * \brief Opacity accessor that utilizes STL-like iterators.  This accessor
   *     expects a list of densities in an STL container.  An opacity value will
   *     be returned for each density provided.  The opacity iterator should be
   *     the same length as the density STL container.
   *
   ...
   *     densities.
   * \param opacityFirst beginning position of a STL container into which
   *     opacity values (corresponding to the provided temperature and density
   *     values) will be stored.
   * \return A list (of type OpacityIterator) of opacities are returned.  These
   *     opacities correspond to the temperature and density values provied.
  ...
  // It is not clear how to assume order of opacity(temp,dens) when accessed in
  // this manner --> for now use the STL-style accessor or a loop over one of
  // the other vector-accessors.
