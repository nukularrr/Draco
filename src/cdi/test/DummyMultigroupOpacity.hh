 ...
 * MultigroupOpacity this class provides three additional 1-D STL-like accessors
 * for opacity data.
 *
...
public:
   ...
   *
   * \sa The constructor assigns fixed values for all of the member data.  Every
   ...
   * \brief Constructor for DummyMultigroupOpacity object.
   *
   * This constructor allows the user to enter a different number of frequency
   * boundaries.
   */
   ...
   *
   * This is required to correctly release memory when a DummyMultigroupOpacity
   * object is destroyed.
   */
  ...
  /*!
   * \brief Opacity accessor that returns a vector of opacities (one for each
   *     group) that corresponds to the provided temperature and density.
   *
   *     Opacity = 2 * ( temperature + density/1000 )
   *                 / ( E_high + E_low )
   *
   ...
   *     density.  Each multigroup opacity is in itself a vector of numGroups
   *     opacities.
   *
   *     Opacity = 2 * ( temperature + density/1000 )
   *                 / ( E_high + E_low )
   ...
   * \param targetDensity The density value for which opacity values are being
   *     requested (g/cm^3).
   * \return A vector of interpolated multigroup opacities (cm^2/g).  Each entry
   *     of this vector corresponds to one of the provided temperatures and is
   ...
   *
   * \return A vector of interpolated multigroup opacities (cm^2/g).  Each entry
   *     of this vector corresponds to one of the provided densities and is
   *     itself a vector of numGroups opacities.
   */
  std::vector<std::vector<double>>
   ...
   *
   * \param tempIter The beginning position of a STL container that holds a list
   *     of temperatures (keV).
   * \param templast The end position of a STL container that holds a list of
   *     temperatures (keV).
   * \param densIter The beginning position of a STL container that holds a list
   *     of densities (g/cm^3).
   * \param densLast The end position of a STL container that holds a list of
   *     densities (g/cm^3).
   * \param opacityIter The beginning position of a STL container into which
   *     multigroup opacity values corresponding to the given tuple of
   *     (temperature, density) values and the number of energy groups will be
   *     stored (cm^2/g).
   ...
   *     provided tuple of (temperature, density) values and the total number of
   *     energy groups.
   */
   ...
   *     temperatures (keV).
   *
   * \param targetDensity A single density value for which opacity values are
   *     being requested (g/cm^3).
   *
   * \param opacityIter The beginning position of a STL container into which
   *     multigroup opacity values corresponding to the given list of
   *     temperature values and the number of energy groups will be stored
   *     (cm^2/g).
   *
   * \return A list (of type OpacityIterator) of multigroup opacities are
   *     returned (cm^2/g).  These multigroup opacities correspond to the
   *     provided list of temperature values, the fixed density and the total
   ...
   *
   * \param opacityIter The beginning position of a STL container into which
   *     multigroup opacity values corresponding to the given list of density
   *     values and the number of energy groups will be stored (cm^2/g).
   *
   * \return A list (of type OpacityIterator) of multigroup opacities are
   *     returned (cm^2/g).  These multigroup opacities correspond to the
   *     provided list of density values, the fixed temperature and the total
   *     number of energy groups.
   */
  ...
  /*!
   * \brief Returns a "plain English" description of the energy group structure
   *	  (gray vs. multigroup).
   */
  ...
  /*!
   * \brief Returns a vector of energy group boundaries that define the cached
 ...
 * should be equal and the length of the opacity container should be numGroups x
 * temperature.size().
 */
template <typename TemperatureIterator, typename DensityIterator,
          typename OpacityIterator>
...
/*!
 * \brief Opacity accessor that returns an STL container of opacities that
 * correspond to a list of provided STL density values and a fixed temperature.
 * The length of the opacity container should be numGroups x density.size().
 */
template <typename DensityIterator, typename OpacityIterator>
OpacityIterator DummyMultigroupOpacity::getOpacity(
