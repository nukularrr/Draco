 ...
 *
 * \brief This is a pure virtual class that defines a standard interface for all
 *        derived MultigroupOpacity objects.
 *
 * Any derived MultigroupOpacity object must provide as a minimum the
 * functionality outlined in this routine.  This functionality includes access
 * to the data grid and the ability to return interpolated opacity values.
 *
...
public:
   ...
   * MultigroupOpacity is destroyed.
   */
  ...
  /*!
   * \brief Opacity accessor that returns a vector of opacities (a single
   *     opacity for each group) that correspond to the provided temperature and
   *     density.
   *
   * \param targetTemperature The temperature value for which these opacity
   *     values are being requested (keV).
   * \param targetDensity The density value for which these opacity values are
  ...
  /*!
   * \brief Opacity accessor that returns a vector of multigroup opacity vectors
   *     that correspond to the provided vector of temperatures and a single
   *     density value.
   *
  ...
  /*!
   * \brief Returns the number of energy groups ( getNumGroupBoundaries() - 1 ).
   */
   ...
   * MultigroupOpacity through a base class pointer, the client must know the
   * derived type when unpacking.
   */
